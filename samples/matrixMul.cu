...
__global__ void matrixMul(float *C, float *A, float *B, int wA, int wB, int bsize) {
  ...
  /*@ ensures \forall i; \forall j;
       0 <= i && i < hA && 0 <= j && j < wB ->
         C[wB * i + j] ==
         sum(k, A[wA * i + k] * B[wB * k + j], 0, wA-1); */
  int bx = blockIdx.x;
  int by = blockIdx.y;
  int tx = threadIdx.x;
  int ty = threadIdx.y;
...
void matrixMul2(float *C, float *A, float *B, int wA, int wB) {
  ...
  /*@ loop invariant loop_count <= m;
      ...
      loop invariant Csub[1] ==
        sum(k, A[wA * (bsize_x * by + (ty + bsize_y)) + k] * B[wB * k + bsize_x * bx + tx],
            0, bsize_x * loop_count - 1); */
  for (int a = aBegin, b = bBegin; a <= aEnd; a += aStep, b += bStep) {
    __shared__ float As[][];
    __shared__ float Bs[][];
  ...
  /*@ ghost int m, hA
      ...
      ensures \forall i1; \forall i2; \forall j1; \forall j2;
        ...
        0 <= j1 && j1 < gridDim.x && 0 <= j2 && j2 < 4 * bsize_y ->
          ...
          sum(k, A[wA * (i1 * 4 * bsize_y + i2) + k] *
              B[wB * k + j1 * 4 * bsize_y + j2], 0, wA-1); */
  int bx = blockIdx.x;
  int by = blockIdx.y;
  int tx = threadIdx.x;
  int ty = threadIdx.y;
...
void matrixMulk(float *C, float *A, float *B, int wA, int wB, int k) {
  /*@ ghost int m, hA, i0, j0;
      ...
      ensures C[wB * i0 + j0] == sum(k, A[wA * i0 + k] * B[wB * k + j0], 0, wA-1); */
  int bx = blockIdx.x;
  int by = blockIdx.y;
  int tx = threadIdx.x;
  int ty = threadIdx.y;
